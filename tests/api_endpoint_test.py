...
def test_journaliser_success(
    ...
    mock_search_cpr.return_value = [{'CPR': '0102030405'}]

    sag_status_id = (
        api_endpoints_module.SBSYS_SAG_STATUS_ACTIVE_TEST
        if api_endpoints_module.TESTING
        else api_endpoints_module.SBSYS_SAG_STATUS_ACTIVE_PROD
    )
    mock_get_personalesag.return_value = [
        {
            'Id': 123,
            'Nummer': 'SAG-1',
            'SagsStatus': {'Id': sag_status_id},
        }
    ]
